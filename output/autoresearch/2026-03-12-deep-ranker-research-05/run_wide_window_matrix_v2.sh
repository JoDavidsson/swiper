...
OUT_DIR="$REPO_ROOT/output/autoresearch/2026-03-12-deep-ranker-research-05"
LOG_DIR="$OUT_DIR/logs"
RESULTS_TSV="$OUT_DIR/results.tsv"

mkdir -p "$LOG_DIR"

printf 'config\toracle_metric\toracle_failed\tlikes_metric\tlikes_failed\tprobe_variant\tprobe_rank_window\tprobe_rank_window_multiplier\tprobe_candidate_count\tprobe_exploration_rate\tprobe_mmr_enabled\tprobe_mmr_applied\tconfig_validation\n' > "$RESULTS_TSV"

run_config() {
  local cfg_line="$1"
  IFS='|' read -r name env_vars expected_window expected_multiplier expected_explore <<< "$cfg_line"
...
INNER
  ...
  local validation="ok"
  if [ "$probe_rank_window" != "$expected_window" ] || [ "$probe_rank_window_multiplier" != "$expected_multiplier" ]; then
    validation="unexpected_window"
  fi
  if [ "$probe_explore" != "$expected_explore" ]; then
    if [ "$validation" = "ok" ]; then
      validation="unexpected_exploration"
    else
      validation="unexpected_window_and_exploration"
    fi
  fi
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$probe_explore" "$probe_mmr_enabled" "$probe_mmr_applied" "$validation" \
    >> "$RESULTS_TSV"

  echo "Done: $name (oracle=$oracle_metric likes=$likes_metric rankWindow=$probe_rank_window validation=$validation)"
}

COMMON_ENVS="RANKER_ENABLE_MMR_RERANK=false RANKER_ADAPTIVE_EXPLORATION_ENABLED=false DECK_ITEMS_FETCH_LIMIT=2000 DECK_CANDIDATE_CAP=1200"

CONFIGS=(
  "wide_win12_exp0|$COMMON_ENVS RANKER_EXPLORATION_RATE=0 DECK_RANK_WINDOW_MULTIPLIER=12|120|12|0"
  "wide_win24_exp0|$COMMON_ENVS RANKER_EXPLORATION_RATE=0 DECK_RANK_WINDOW_MULTIPLIER=24|240|24|0"
  "wide_win48_exp0|$COMMON_ENVS RANKER_EXPLORATION_RATE=0 DECK_RANK_WINDOW_MULTIPLIER=48|480|48|0"
  "wide_win72_exp0|$COMMON_ENVS RANKER_EXPLORATION_RATE=0 DECK_RANK_WINDOW_MULTIPLIER=72|720|72|0"
  "wide_win24_exp8|$COMMON_ENVS RANKER_EXPLORATION_RATE=0.08 DECK_RANK_WINDOW_MULTIPLIER=24|240|24|0.08"
  "wide_win48_exp8|$COMMON_ENVS RANKER_EXPLORATION_RATE=0.08 DECK_RANK_WINDOW_MULTIPLIER=48|480|48|0.08"
)
...
echo "=== Wide window matrix v2 complete ==="
