...
OUT_DIR="$REPO_ROOT/output/autoresearch/2026-03-12-deep-ranker-research-06"
LOG_DIR="$OUT_DIR/logs"
RESULTS_TSV="$OUT_DIR/results.tsv"

mkdir -p "$LOG_DIR"

printf 'config\toracle_metric\tlikes_metric\tprobe_rank_window\tprobe_candidate_count\tlatency_count\tlatency_avg_ms\tlatency_p95_ms\tlatency_max_ms\n' > "$RESULTS_TSV"
...
INNER
  ...
  local probe_rank_window probe_candidate_count
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$latency_count" "$latency_avg" "$latency_p95" "$latency_max" \
    >> "$RESULTS_TSV"

  echo "Done: $name (oracle=$oracle_metric likes=$likes_metric avgLatencyMs=$latency_avg p95Ms=$latency_p95)"
}
...
CONFIGS=(
  "cap240_fetch360|$COMMON_ENVS DECK_CANDIDATE_CAP=240 DECK_ITEMS_FETCH_LIMIT=360"
  "cap400_fetch700|$COMMON_ENVS DECK_CANDIDATE_CAP=400 DECK_ITEMS_FETCH_LIMIT=700"
  "cap800_fetch1400|$COMMON_ENVS DECK_CANDIDATE_CAP=800 DECK_ITEMS_FETCH_LIMIT=1400"
  "cap1200_fetch2000|$COMMON_ENVS DECK_CANDIDATE_CAP=1200 DECK_ITEMS_FETCH_LIMIT=2000"
)
...
echo "=== Cap/latency matrix complete ==="
